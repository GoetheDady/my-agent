...
export function parseDbContent(contentStr: string, role: "user" | "assistant"): Array<{ type: string; text?: string; toolInvocation?: { toolName: string; args: Record<string, unknown> }; reasoning?: string }> {
  if (role === "user") {
    try {
  ...
  try {
    const blocks = JSON.parse(contentStr) as Array<{ type: string; text?: string; thinking?: string; name?: string; id?: string; input?: Record<string, unknown>; toolInvocation?: { toolName: string; args: Record<string, unknown> } }>;
    return blocks.map((b) => {
      if (b.type === "text") return { type: "text", text: b.text ?? "" };
      if (b.type === "thinking") return { type: "reasoning", text: b.thinking ?? b.text ?? "", reasoning: b.thinking ?? b.text ?? "" };
