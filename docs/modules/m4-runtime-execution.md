...
- Web runner 和 internal runner 在构建 system prompt 前会用当前 task input 检索最多 5 条相关长期记忆，再交给 Prompt & Context 模块注入。
...
本轮 RAG-in-context 改动对 Runtime 的影响：

- `runAgentTask()` 改为异步函数，因为模型调用前需要等待 prompt 和记忆检索上下文准备完成。
- `agent-runtime.ts` 和 `internal-runner.ts` 都会在 `building_prompt` 阶段执行相关长期记忆检索。
- 记忆检索失败不会把 task 标记失败；Runtime 会降级为空记忆上下文后继续调用模型。
- RAG-in-context 指“检索增强的上下文注入”：先从长期记忆中查出相关片段，再把片段放入本轮模型上下文。
